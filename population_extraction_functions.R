...
extract_wld <-function(filenames)
{
  ...
  #save country values as .gpkg
  st_write(x_4, dsn =paste0("./Output_pop/Coral_pop_ls",regmatches(filenames, regexpr("[0-9].*[0-9]",filenames)),"_countries.gpkg"), delete_dsn =TRUE)
  #drop geometry
  x_5 <-st_drop_geometry(x_4)
  #save values as .csv
  write.csv(x_5, file = paste0("./Output_pop/Pop_ls",regmatches(filenames, regexpr("[0-9].*[0-9]",filenames)),"_countries.csv"))
}
